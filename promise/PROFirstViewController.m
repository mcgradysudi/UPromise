...
@interface PROFirstViewController ()
...
static NSString *detailTaskCellIndent = @"detailTaskCellIndent";

@implementation PROFirstViewController
...
- (void)viewDidLoad
{
    ...
    [self.tableView registerClass:[PROUserPromiseListTableViewCell class] forCellReuseIdentifier:detailTaskCellIndent];
...
-(void)initUserHeadView
{
    ...
    self.userHeaderView.viewFriend = self.viewFriend;
...
-(void)initTableResult
{
    ...
    if (self.selectTag == 0) {
        NSPredicate *selectPredict = [NSPredicate predicateWithFormat:@"user_id==%@" ,self.viewFriend.userid,self.selectTag];
        [fetchRequest setPredicate:selectPredict];
    }
    else if(self.selectTag == 1)
    {
        NSPredicate *selectPredict = [NSPredicate predicateWithFormat:@"user_id==%@ AND pro_status==%d" ,self.viewFriend.userid,self.selectTag];
        [fetchRequest setPredicate:selectPredict];
    }
    else if(self.selectTag == 2)
    {
        NSPredicate *selectPredict = [NSPredicate predicateWithFormat:@"watch_id==%@" ,self.viewFriend.userid];
        [fetchRequest setPredicate:selectPredict];
    }
...
{
    PROUserPromiseListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:detailTaskCellIndent forIndexPath:indexPath];
    //    if (cell==nil) {
...
{
    self.userHeaderView.allLabel.text = [NSString stringWithFormat:@"所有 %@",self.viewFriend.totalsize];
    
    
    
    self.userHeaderView.completeLabel.text = [NSString stringWithFormat:@"完成  %@",self.viewFriend.successsize];
    
    self.userHeaderView.watchLabel.text = [NSString stringWithFormat:@"监督  %@",self.viewFriend.watchsize];
...
- (void)headerRereshing
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        NSArray * promiseInfo = [self.myMessageData getFriendPromiseFromId:1 andPageNum:30 userID:self.viewFriend.userid.integerValue andPromiseState:self.selectTag?(self.selectTag==2?3:self.selectTag-1):-1];
        //:1 andPageNum:30 andPromiseState:self.selectTag?(self.selectTag==2?3:self.selectTag-1):-1];
        if (promiseInfo&&[promiseInfo count]>0) {
            dispatch_async(dispatch_get_main_queue(), ^{
                //回调或者说是通知主线程刷新，
                [self.appdelgate initPromiseInfo:promiseInfo andWatchId:self.viewFriend.userid andState:self.selectTag?(self.selectTag==2?3:self.selectTag-1):-1];
                // [self initTableResult];
            });
        }
        
        if ([self.myMessageData getPromiseSize:self.viewFriend.userid]) {
...
- (void)footerRereshing
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        NSArray * promiseInfo =  [self.myMessageData getFriendPromiseFromId:++page andPageNum:30 userID:(self.viewFriend.userid.integerValue) andPromiseState:self.selectTag];
        //:++page andPageNum:30 andPromiseState:self.selectTag] ;
        if (promiseInfo&&[promiseInfo count]>0) {
            dispatch_async(dispatch_get_main_queue(), ^{
                //回调或者说是通知主线程刷新，
                [self.appdelgate initPromiseInfo:promiseInfo andWatchId:self.viewFriend.userid andState:self.selectTag];
                // [self initTableResult];
            });
        }
